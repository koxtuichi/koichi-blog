    ...
    <>
      ...
      <Container>
        <Form onSubmit={handleSubmit}>
          <Grid centered>
            <ContainerCenter>
              <p style={{ marginBottom: '8p' }}>どんな写真が撮りたいですか？<br />「お花」や「カフェ」などの撮りたいモノの名前を10文字以内で入れてみてください。<br />関東でおすすめのスポットをAIが探してくれます。</p>
              <Input
                type="input"
                name="searchWord"
                ...
                maxLength={10}
              />
              <Button type="submit" disabled={loading}>探す</Button>
              {0 < response.split(",").length && (
                <>
                  {response.split(",").map((item, index) => (
                    <p key={index} style={{ marginTop: '8px' }}>{item}</p>
                  ))}
                </>
              )}
            </ContainerCenter>
