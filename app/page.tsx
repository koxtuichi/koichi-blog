...
const Home = async () => {
	...
	posts.map(async (post) => {
		...
		const binary2 = blob2 ? (await blob2.arrayBuffer()) as Uint8Array : undefined;
		const binary3 = blob3 ? (await blob3.arrayBuffer()) as Uint8Array : undefined;
		const binary4 = blob4 ? (await blob4.arrayBuffer()) as Uint8Array : undefined;
		...
		buffer2 && fs.writeFile(savePath2, buffer2, (error) => {
			if (error) {
				throw error;
			}
		});
		buffer3 && fs.writeFile(savePath3, buffer3, (error) => {
			if (error) {
				throw error;
			}
		});
		buffer4 && fs.writeFile(savePath4, buffer4, (error) => {
			if (error) {
				throw error;
			}
		});
